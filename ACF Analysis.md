...
> Neptune, Pluto, FPOA) plus the mean north lunar node **Rahu**, with
...
> precession has carried to ~33.5° tropical, in early Taurus). The
> recovered absolute longitude is 0.497° ± 0.012° across the 290-day
> window — unambiguously the equinox, ~33° away from γ Arietis. The
> labelling has been corrected to **FPOA** (First Point of Aries) here
> and throughout the paper for accuracy.
...
| --------- | ----------------------: | -----------: | ----: | ----------------------------------- |
| FPOA      | **9**                   | 0.546        | 267.2 | strong + at 24–72°, − at 120°, 180° |
...
astrological lags clear the white-noise threshold, the same count as FPOA,
...
| --------- | ----: | -----------: | ------------: | --------------: | ---------------: |
| FPOA      | 267.2 | 0.0058       | **86.2**      | 0.0186          | **26.9**         |
...
FPOA, Pluto, and Neptune reach "very strong" Bayes factors under the
...
> modern planets, Pluto, the First Point of Aries (FPOA) (a proxy for the First Point
...
> (FPOA) under the compound null and up to 27 under the unnamed-MBA null,
> with FPOA, Pluto, Neptune, Uranus, Saturn, Rahu, and Jupiter all reaching
...
> (Saturn through Pluto and the FPOA reference), with per-SP BF = 19.2 (compound) and
> 9.65 (unnamed-MBA)."
